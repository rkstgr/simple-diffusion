# Copied from https://github.com/lucidrains/denoising-diffusion-pytorch/blob/main/denoising_diffusion_pytorch/simple_diffusion.py
...
class UViT(nn.Module):
    def __init__(
        ...
        assert isinstance(init_dim, int) and (init_dim > 0), 'initial dimension must be a positive integer'
...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, time, time_next):
        ...
        if self.pred_objective == 'v':
            ...
            raise NotImplementedError(f"pred_objective must be 'v' or 'eps', got {self.pred_objective}")
